...
#         train_w = weight[None,mask.to(torch.bool)]    
        ...
        correct_t = -np.square(train_pred.argmax(-1) - train_lab.argmax(-1)).float().mean().item()

        #GRAPH LOSS
        if logprobs is not None: 
            corr_pred = -np.square(train_pred.argmax(-1)-train_lab.argmax(-1)).float().detach()
    ...
    def test_step(self, train_batch, batch_idx):
        ...
        correct_t = -np.square(test_pred.argmax(-1) - test_lab.argmax(-1)).float().mean().item()
