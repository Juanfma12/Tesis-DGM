...
#         train_w = weight[None,mask.to(torch.bool)]    
        ...
        loss = torch.nn.functional.mse_loss(train_pred,train_lab)
        loss.backward()

        correct_t = -torch.square(train_pred.cpu() - train_lab.cpu()).float().mean().item()
        
        #GRAPH LOSS
        if logprobs is not None: 
            corr_pred = -torch.square(train_pred.cpu()-train_lab.cpu()).float().detach()
            ...
            point_w = point_w.to(logprobs.device)
            graph_loss = point_w * torch.unsqueeze(logprobs.exp().mean([-1,-2]),dim = -1)
        ...
        self.log('train_acc', abs(correct_t))
    ...
    def test_step(self, train_batch, batch_idx):
        ...
        test_pred = pred
        test_lab = y
        correct_t = -torch.square(test_pred.cpu() - test_lab.cpu()).float().mean().item()
        loss = torch.nn.functional.mse_loss(test_pred,test_lab)
        self.log('test_loss', loss.detach().cpu())
        # self.log('test_graph_loss', loss.detach().cpu())
        self.log('test_acc', abs(correct_t))
    ...
    def validation_step(self, train_batch, batch_idx):
        ...
        self.log('val_acc', abs(correct_t))
